...
export const step2Schema = z.object({
  ...
  coordinatorMobile: z.string().length(10, "Mobile must be 10 digits").regex(/^\d+$/, "Mobile must contain only digits"),
});
...
export const licenseSchema = z.object({
  ...
  (data) => {
    if (!data.issueDate || !data.expiryDate) return true; // Skip if dates not provided
...
// Combined schema for final review 
