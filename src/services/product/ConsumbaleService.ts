...
export const getConsumableSpecificationUnitsBySubCategory = async (subCategoryId: string | number) => {
  try {
    const response = await api.get(`masters/by-subcategory/${subCategoryId}`);
    return response.data?.data ?? response.data ?? [];
  } catch (error: unknown) {
    console.error("Error fetching specification units:", error);
    if (error instanceof Error) throw new Error(`Error fetching specification units: ${error.message}`);
