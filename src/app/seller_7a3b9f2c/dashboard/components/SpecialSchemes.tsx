  ...
  const validateForm = (
    ...
    if (!fieldName || fieldName === "effectiveStartDate") {
      if (!updatedForm.effectiveStartDate) {
        ...
        if (startDate < today) {
          newErrors.effectiveStartDate = "Start Date cannot be less than today";
        }
      }
    }
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <th
                        key={header.id}
                        className="px-4 py-3 text-left text-label-l2 font-semibold text-neutral-900 border border-neutral-300"
                      >
                        {flexRender(
