...
interface ProductApiData {
  ...
  productAttributeFoodInfants?: FoodInfantAttributes[];
  productAttributeFoodInfant?: FoodInfantAttributes;
...
const ProductView1 = ({
  ...
  useEffect(() => {
    ...
    // ── Food & Infant: product form ──
    const formId = toPositiveInt(foodAttr?.productFormId);
    const inlineFormName = foodAttr?.productFormName?.trim() || null;

    const fetchProductForm = async (): Promise<Partial<ResolvedLookups>> => {
      if (inlineFormName) return { productFormName: inlineFormName };
      if (!formId) return {};
      try {
        const formData = await getProductForms();
        const found = formData.find((f: any) => Number(f.productFormId ?? f.id) === formId);
        return { productFormName: found?.productForm ?? found?.name ?? null };
      } catch {
        return {};
      }
    };

    // ── Food & Infant: age group ──
    const ageId = toPositiveInt(foodAttr?.ageGroupId);
    const inlineAgeName = foodAttr?.ageGroupName?.trim() || null;

    const fetchAgeGroup = async (): Promise<Partial<ResolvedLookups>> => {
      if (inlineAgeName) return { ageGroupName: inlineAgeName };
      if (!ageId) return {};
      try {
        const ageData = await getAgeGroups();
        const found = ageData.find((a: any) => Number(a.ageGroupId ?? a.id) === ageId);
        return { ageGroupName: found?.ageGroup ?? found?.name ?? null };
      } catch {
        return {};
      }
    };
    ...
    // ── Food & Infant: country ──
    const countryId = toPositiveInt(foodAttr?.countryId);
    const inlineCountryName = foodAttr?.countryName?.trim() || null;

    const fetchCountry = async (): Promise<Partial<ResolvedLookups>> => {
      if (inlineCountryName) return { countryName: inlineCountryName };
      if (!countryId) return {};
      try {
        const countryData = await getCountries();
        const found = countryData.find((c: any) => Number(c.countryId ?? c.id) === countryId);
        return { countryName: found?.countryName ?? found?.name ?? null };
      } catch {
        return {};
      }
    };

    // ── Food & Infant: categories ──
    const prodCatId = toPositiveInt(foodAttr?.productCategoryId);
    const prodSubId = toPositiveInt(foodAttr?.productSubcategoryId);
    const inlineCatNameFood = foodAttr?.productCategoryName?.trim() || null;
    const inlineSubNameFood = foodAttr?.productSubcategoryName?.trim() || null;

    const fetchProductCategories = async (): Promise<Partial<ResolvedLookups>> => {
      let productCategoryName = inlineCatNameFood;
      let productSubcategoryName = inlineSubNameFood;

      if (!productCategoryName && prodCatId) {
        try {
          const catData = await getProductCategories(3);
          const found = catData.find((c: any) => Number(c.productCategoryId ?? c.id) === prodCatId);
          productCategoryName = found?.productCategory ?? found?.name ?? null;
        } catch {}
      }

      if (!productSubcategoryName && prodCatId && prodSubId) {
        try {
          const subData = await getProductSubcategories(prodCatId);
          const found = subData.find((s: any) => Number(s.productSubcategoryId ?? s.id) === prodSubId);
          productSubcategoryName = found?.productSubcategory ?? found?.name ?? null;
        } catch {}
      }

      return { productCategoryName, productSubcategoryName };
    };
  ...
  const foodAttrRaw: FoodInfantAttributes | null = productData
    ? (productData.productAttributeFoodInfants?.[0] || productData.productAttributeFoodInfant || null)
    : null;
  ...
  const storageCondition: string | null =
    ...
    foodAttrRaw?.storageConditionName ??
  ...
  const brochureUrl: string | null =
    ...
    validUrl(foodAttrRaw?.productUserManual) ??
  ...
  // Prepare FoodInfantView props
const foodInfantViewProps = isFoodInfant && foodAttrRaw && productData ? {
  productName: productData.productName,
  productDescription: productDescription,
  warningsPrecautions: warningsPrecautions,
  displayImages: displayImages,
  foodAttr: {
  productCategoryName: lookups.productCategoryName ?? undefined,
  productSubcategoryName: lookups.productSubcategoryName ?? undefined,
  brandName: foodAttrRaw.brandName,
  variantName: foodAttrRaw.variantName,
  productFormName: lookups.productFormName ?? undefined,
  netQuantity: foodAttrRaw.netQuantity,
  servingSize: foodAttrRaw.servingSize,
  ageGroupName: lookups.ageGroupName ?? undefined,
  vegNonvegIndicator: foodAttrRaw.vegNonvegIndicator,
  allergenInformation: foodAttrRaw.allergenInformation,
  nutritionalInformation: foodAttrRaw.nutritionalInformation,
  nutritionalInformationImageUrl: foodAttrRaw.nutritionalInformationImageUrl,
  activeIngredients: foodAttrRaw.activeIngredients,
  additivesPreservatives: foodAttrRaw.additivesPreservatives,
  productClaims: foodAttrRaw.productClaims,
  storageConditionName: storageCondition ?? undefined,
  manufacturerName: manufacturerName,
  countryName: lookups.countryName ?? undefined,
  certificateDocuments: foodAttrRaw.certificateDocuments,
  productUserManual: foodAttrRaw.productUserManual,
},
  storageConditionName: storageCondition ?? undefined,
  brochureUrl: brochureUrl,
  placeholderImage: PLACEHOLDER_IMAGE,
  packagingDetails: packaging ? {
    packTypeName: resolvedPackType ?? undefined,
    unitsPerPack: packaging.unitPerPack ?? packaging.unitsPerPack,
    numberOfPacks: packaging.numberOfPacks,
    packSize: packaging.packSize,
    minimumOrderQuantity: packaging.minimumOrderQuantity,
    maximumOrderQuantity: packaging.maximumOrderQuantity,
  } : null,
  pricingDetails: pricing ? {
    mrp: pricing.mrp,
    sellingPrice: pricing.sellingPrice,
    discountPercentage: pricing.discountPercentage,
    finalPrice: pricing.finalPrice ?? undefined,
    batchLotNumber: pricing.batchLotNumber ?? undefined,
    manufacturingDate: pricing.manufacturingDate ?? undefined,
    expiryDate: pricing.expiryDate ?? undefined,
    stockQuantity: pricing.stockQuantity,
    dateOfStockEntry: pricing.dateOfStockEntry ?? undefined,
    shelfLife: shelfLifeDisplay ?? undefined,
    gstPercentage: gstPercentage ?? undefined,
    hsnCode: hsnCode,
  } : null,
  taxDetails: {
  ...
  return (
    ...
    >
      ...
      {isFoodInfant && foodInfantViewProps && (
        <FoodInfantView {...foodInfantViewProps} />
      )}
      ...
      {/* ── PACKAGING & ORDER DETAILS ──
          Only show for Drug and Supplement (since other views have their own packaging sections) */}
      {(isDrug || isSupplement) && (
        <div style={{ alignSelf: "stretch", display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {/* ── BATCH MANAGEMENT + PRICING ──
          Only show for Drug and Supplement (since other views have their own sections) */}
      {(isDrug || isSupplement) && (
