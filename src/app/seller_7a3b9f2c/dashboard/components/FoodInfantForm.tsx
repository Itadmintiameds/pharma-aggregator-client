...
const [certificateDocumentIds, setCertificateDocumentIds] = useState<Map<number, number>>(new Map());
 ...
 const handleCertificationSelectionChange = (selectedValues: string[]) => {
  // In edit mode, prevent removal of any existing certifications
  if (isEditMode && selectedCertificationValues.length > 0) {
    ...
    // If ANY certification with existingUrl is being removed, block it
    // (existingUrl includes PENDING/NOT_UPLOADED for Excel imports)
    const hasExistingRemoved = removedCerts.some(removedId => {
      const cert = certificationsDetails.find(c => c.id === removedId);
      return cert?.existingUrl !== undefined && cert?.existingUrl !== "";
    });
 ...
 const handleCertRemove = (certId: string) => {
  ...
  // Prevent removal if:
  // 1. In edit mode AND the certification has any existingUrl (including PENDING/NOT_UPLOADED)
  //    This covers both Excel-imported (PENDING/NOT_UPLOADED) and manually uploaded (real URLs)
  if (isEditMode && cert?.existingUrl) {
    setErrors((prev) => ({
      ...prev,
      certifications: "Existing certifications cannot be removed."
    }));
    return;
  }
  
  // For new certifications (no existingUrl) in edit mode OR any certification in create mode
  setCertificationsDetails((prev) =>
  ...
  const validateCertifications = (): string | null => {
    ...
    const missingFiles = certificationsDetails.filter((cert) => {
      // Check if there's NO valid uploaded file
      const hasValidFile = cert.isUploaded;
      const hasValidExistingUrl = cert.existingUrl && 
        cert.existingUrl !== "NOT_UPLOADED" && 
        cert.existingUrl !== "PENDING";
      
      return !hasValidFile && !hasValidExistingUrl;
    });
...
// Real-time validation for Warnings & Precautions
if (name === "warningsPrecautions") {
  const trimmedValue = value.trim();
  if (!trimmedValue) {
    ...
    setErrors((prev) => {
      const newErrors = { ...prev };
      delete newErrors.warningsPrecautions;
      return newErrors;
    });
  }
}
...
if ((attr as any).certificateDocuments?.length) {
  ...
  const validateForm = (): Record<string, string> => {
    ...
    if (!form.warningsPrecautions.trim()) {
  ...
  if (newProductAttributeId && certificationsDetails.length > 0) {
    ...
    const certDocsFromResponse = updatedData?.productAttributeFoodInfants?.[0]?.certificateDocuments || [];
    
    // Create a map of certificationId -> productCertificateDocumentId
    const docIdMap = new Map<number, number>();
    certDocsFromResponse.forEach((doc: any) => {
      docIdMap.set(doc.certificationId, doc.productCertificateDocumentId);
    });
    
    // Get only new certificates (with files and no existingUrl)
    const newCertificates = certificationsDetails.filter(
      (cert) => cert.file && !cert.existingUrl
    );
    
    if (newCertificates.length > 0) {
      try {
        const files = newCertificates.map((cert) => cert.file as File);
        const documentIds = newCertificates
          .map((cert) => docIdMap.get(Number(cert.id)))
          .filter((id): id is number => id !== undefined);
        
        if (files.length === documentIds.length && files.length > 0) {
          await uploadFoodInfantCertificates(newProductAttributeId, files, documentIds);
          console.log("✅ Certificates uploaded successfully in edit mode");
        }
      } catch (certErr: any) {
        console.warn("⚠️ Certificates upload failed in edit mode:", certErr.message);
      }
    }
  }
      } else {
  ...
  try {
    ...
    console.warn("⚠️ Nutritional image upload failed:", err);
      ...
      if (newProductAttributeId && certificationsDetails.length > 0) {
        // Get the certificate documents from response to get document IDs
        const certDocsFromResponse = response?.data?.productAttributeFoodInfants?.[0]?.certificateDocuments || [];
        ...
        if (newCertificates.length > 0) {
          try {
            ...
            if (files.length === documentIds.length && files.length > 0) {
              ...
              console.warn("⚠️ Mismatch between files and document IDs");
            }
          } catch (certErr: any) {
            console.warn("⚠️ Certificates upload failed:", certErr.message);
          }
        }
      }
...
{selectedCertificationValues.map((certId) => {
  ...
  {!errors.productDescription && form.productDescription.length > 0 && form.productDescription.length < 10 && (
  ...
  {!errors.warningsPrecautions && form.warningsPrecautions.length > 0 && form.warningsPrecautions.length < 10 && (
    <p className="text-warning-500 text-sm mt-1">
