...
const ConsumableForm = ({ productId, mode = "create", onSubmitSuccess }: ConsumableFormProps) => {
  const [form, setForm] = useState({
    ...
    shelfLife: "", intendedUse: "", keyFeatures: "", safetyInstructions: "",
    countryOfOrigin: "", manufacturerName: "", storageCondition: "",
  ...
  const sterileOptions: SelectOption[] = [{ value: "sterile", label: "Sterile" }, { value: "non-sterile", label: "Non-Sterile" }];
  const disposableOptions: SelectOption[] = [{ value: "disposable", label: "Disposable" }, { value: "reusable", label: "Reusable" }];
  const gstOptions: SelectOption[] = [{ value: "0", label: "0%" }, { value: "5", label: "5%" }, { value: "12", label: "12%" }, { value: "18", label: "18%" }];
  ...
  const fetchList = useCallback(async (url: string, setter: (v: SelectOption[]) => void, idKey: string[], labelKey: string[], fallback?: SelectOption[]) => {
    try {
      ...
      const mapped = items.map((item) => ({ value: getMasterStr(item, ...idKey), label: getMasterStr(item, ...labelKey) || "Unknown" })).filter((o) => o.value);
      setter(mapped);
  ...
  const validate = (): Record<string, string> => {
    ...
    if (!form.productName.trim() || form.productName.trim().length < 3) e.productName = "Product name required (minimum 3 characters)";
    ...
    if (!form.intendedUse.trim() || form.intendedUse.trim().length < 10) e.intendedUse = "Intended use required (minimum 10 characters)";
    if (!form.keyFeatures.trim() || form.keyFeatures.trim().length < 10) e.keyFeatures = "Key features required (minimum 10 characters)";
    if (!form.safetyInstructions.trim() || form.safetyInstructions.trim().length < 10) e.safetyInstructions = "Safety instructions required (minimum 10 characters)";
    ...
    if (!form.productDescription.trim() || form.productDescription.trim().length < 20) e.productDescription = "Product description required (minimum 20 characters)";
    ...
    if (isNaN(minQ) || minQ <= 0) e.minimumOrderQuantity = "Min order qty must be > 0";
    const maxQ = parseFloat(form.maximumOrderQuantity);
    if (isNaN(maxQ) || maxQ <= 0) e.maximumOrderQuantity = "Max order qty must be > 0";
    ...
    <div className="flex flex-col gap-5 max-w-full mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-8 gap-y-4">
                        ...
                        {cert.existingUrl && !cert.file ? (
                          <div className="flex items-center border border-purple-300 rounded-xl overflow-hidden h-14 bg-purple-50">
                            ...
                            <div className="flex items-center gap-1 pr-3">
                              <button type="button" onClick={() => document.getElementById(`consumable-cert-upload-${cert.id}`)?.click()} className="p-1.5 rounded-lg hover:bg-purple-200 text-purple-700" title="Replace"><RefreshCw size={14} /></button>
          ...
          <div className="flex flex-col gap-1">
            ...
            <Select options={storageConditionOptions} value={storageConditionOptions.find((o) => o.value === form.storageCondition) || null}
              onChange={(sel) => handleSelectChange("storageCondition", sel)} placeholder="Select storage condition" theme={selectTheme} styles={selectStyles("storageCondition")} />
          ...
          <div className="col-span-1 md:col-span-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-label-l3 text-neutral-700 font-semibold mb-1">Key Features & Specifications <span className="text-warning-500 ml-1">*</span></label>
      ...
      <div className="border border-neutral-200 rounded-xl p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 mt-8">
          ...
          <Input label="Maximum Order Quantity" name="maximumOrderQuantity"
            onChange={handleChange} value={form.maximumOrderQuantity} error={errors.maximumOrderQuantity} required />
        </div>

        <div className="mb-4"><div className="text-h6 text-neutral-900 font-regular mb-2">Batch & Stock Management</div><div className="border-b border-neutral-200" /></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <Input label="Batch / Lot Number" name="batchLotNumber" onChange={handleChange} value={form.batchLotNumber} error={errors.batchLotNumber} required />
