...
export default function AdminLogin({ onClose }: AdminLoginProps) {
  ...
  return (
    // Main overlay - closes modal when clicked
    <div 
      className="fixed inset-0 z-[60] bg-black/60"
      onClick={onClose}
    >
      {/* Main container - prevents closing when clicked inside */}
      <div 
        className="min-h-screen flex items-center justify-center bg-[#F3ECF8] p-4"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Watermark Logo Background - EXACTLY as in original */}
        <div
        ...
        {/* Content Layer - EXACTLY as in original */}
        <div className="flex flex-col items-center gap-6 relative z-10">
          {/* Logo - EXACTLY as in original with Link to home */}
          <Link 
            href="/"
            onClick={(e) => {
              e.stopPropagation();
              onClose(); // Also close the modal when going to home
            }}
          >
            <img
          ...
          {/* Login Form - EXACTLY as in original */}
          <form
